...
export function setupParadeDashboard(container) {
  ...
  return {
    ...
    getContingentsByType(type) {
      const items = container.querySelectorAll(`.contingent[data-type="${type}"]`)
    ...
    getParadeOrder() {
      const items = container.querySelectorAll(".contingent")
      return Array.from(items).map(el => el.dataset.name)
    },

    getTotalMembers() {
      const items = container.querySelectorAll(".contingent li")
      return items.length
    }
  }
}